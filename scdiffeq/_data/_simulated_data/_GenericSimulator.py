...
# local imports #
# ------------- #
from ._simulation_supporting_functions._get_initial_conditions import (
    _get_initial_conditions,
)
from ._simulation_supporting_functions._get_numpy_distribution_function import (
    _get_numpy_distribution_function,
)
from ._simulation_supporting_functions._print_verbose_function_documentation import (
...
from ._simulation_supporting_functions._formatting_simulation_for_AnnData import (
    _simulation_to_AnnData,
)
...
class _GenericSimulator:
    ...
    def set_initial_conditions_sampling_distribution(
        ...
        self.distrubution_function = _get_numpy_distribution_function(
            function, module=module, package=package, print_verbose_documentation=False,
        )
    ...
    def print_distribution_function_docs(self,):
    ...
    def get_initial_conditions(
        self, plot=False, n_bins=20, savefigname=False, **kwargs
    ):
    ...
    def create_time_vector(self, time_span=10.0, n_samples=1000, noise_amplitude=0):
        self.time_vector = _create_time_vector(
            time_span=time_span, n_samples=n_samples, noise_amplitude=noise_amplitude
        )

    def simulate_ODE(
        self, state_function, to_adata=True, plot=True, savefigname=False, **kwargs
    ):
        ...
        print(
            "\nSimulating {} trajectories from {} over {} time points in each trajectory...".format(
                self.n_traj, state_function, len(self.time_vector)
            )
        )
        ...
        if plot:
            _plot(
                ...
                s=12,
                alpha=0.5,
                savefigname=self.simulation_plot_fig_path,
                **kwargs
            )
