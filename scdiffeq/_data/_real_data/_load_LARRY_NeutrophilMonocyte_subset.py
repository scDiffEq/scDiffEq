...
def _download_LARRY_NeutrophilMonocyte_subset(
    destination=os.path.join(os.getcwd(), "data/LARRY_NM_subset/")
):
    ...
    gdrive_file_ids = [
        "1-OzHs09gSOg-4fpvEU3ys0OshL5bHk8e",
        "1J7LVXv8tzRFIQQtaMgJYmPCvp6PByK3C",
        "1NBTnnoaVa4ShKP55mfzbg6Dbt7r5AOm_",
        "1fDoGJIaIE8AYyh37kKFTY_Hll3CdNdnR",
    ]
...
def _load_previously_cached_LARRY_NM_subset_data(cached_data_path):
    ...
    for n, file in enumerate(tqdm.notebook.tqdm(files)):
        ...
        if n == 0:
            print(
                "Returning data as a dictionary of torch.Tensors with the following keys.\n"
            )
