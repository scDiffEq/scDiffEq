...
from ._forward_integration_functions._individual_trajectories._format_trajectories_into_batches import (
    _format_trajectories,
)
from ._forward_integration_functions._individual_trajectories._forward_integrate_one_trajectory import (
    _forward_integrate_one_trajectory,
)
...
def _evaluate(adata):
    ...
    adata.uns["test_trajectories"] = test_trajectories = _format_test_trajectories(
        adata
    )
    adata.uns["test_y_predicted"], adata.uns["test_y"] = _get_batch_outs_empty_tensor(
        test_trajectories
    )
    ...
    for n, trajectory in enumerate(test_trajectories.keys()):
        ...
        with torch.no_grad():
            (
                ...
                adata.uns["ODE"], test_trajectories[trajectory]
            )
    ...
    print(
        "MSELoss: {:.4f}".format(
            adata.uns["test_accuracy"].item() / (len(test_trajectories))
        )
    )
