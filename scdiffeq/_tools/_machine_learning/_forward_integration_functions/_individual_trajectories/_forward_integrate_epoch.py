from ._format_trajectories_into_batches import (
    _get_formatted_training_validation_trajectories,
)
...
def _forward_integrate_epoch(self, epoch):
    ...
    batches = _get_formatted_training_validation_trajectories(
        self.adata, self.n_batches
    )
    ...
    self.adata.uns["loss"]["train_loss"].append(
        epoch_train_loss.item() / n_train_trajs_
    )
