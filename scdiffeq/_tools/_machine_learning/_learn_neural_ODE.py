...
from ._forward_integration_functions._parallel_batch_time._forward_integrate_epoch_parallel_time import _forward_integrate_epoch_parallel_time
from ._forward_integration_functions._individual_trajectories._forward_integrate_epoch import _forward_integrate_epoch
...
def _learn_neural_ODE(
    ...
    for epoch in tqdm((range(1, n_epochs + 1)), desc="Training progress", bar_format=bar_format):
        ...
        if mode == "parallel":
            _forward_integrate_epoch_parallel_time(self, epoch, time_column=time_column, n_batches=n_batches)
        else:
