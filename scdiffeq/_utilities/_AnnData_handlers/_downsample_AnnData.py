...
def _downsample_AnnData(
    ...
    if n_traj:
        trajectory_subset = np.random.choice(adata.obs.trajectory.unique(), n_traj).tolist()
        adata = adata[
            adata.obs.loc[adata.obs.trajectory.isin(trajectory_subset)].index.astype(int)
        ]
