...
def _echo_loss(loss_df, status_file, epoch, dry, silent=False):
    ...
    for subset in cols:
        ...
        if not dry:
            status_file.update(epoch, epoch_loss=loss_df[subset].tolist(), mode=subset)

def _loss(X_pred, optimizer, loss_function, test, epoch, status_file, dry, silent=False):
    ...
    _echo_loss(summarized_loss_df, status_file, epoch, dry, silent)
