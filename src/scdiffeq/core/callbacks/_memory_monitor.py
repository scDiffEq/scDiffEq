...
class MemoryMonitor(lightning.pytorch.callbacks.Callback):
    ...
    def _log_cpu_ram(self, trainer):
        ...
        logger.debug(f"[MemoryLogger] Epoch {trainer.current_epoch}: CPU RAM: {mem:.2f} MB")
    ...
    def _log_gpu_ram(self, trainer, lit_module):
        ...
        if self.log_gpu and lit_module.device.type == "cuda":
            ...
            logger.debug(f"[MemoryLogger] Epoch {trainer.current_epoch}: GPU Allocated: {mem_allocated:.2f} MB | Reserved: {mem_reserved:.2f} MB")
            wandb.log(
                {
    ...
    def _report_object_memories(self, trainer):
        objs = gc.get_objects()
        logger.debug(f"[MemoryLogger] Epoch {trainer.current_epoch}: Number of tracked objects: {len(objs)}")
